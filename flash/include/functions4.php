...
function get_video_tags($uniq_id = '', $make_links = 1)
{
	...
	while($row = mysql_fetch_assoc($result))
	{
		...
		if($make_links != 0)
		{
			if(_SEOMOD == 1)
			{
				$tags[$id]['href'] ="<a " . 'style="background-color:#3881FF;color:#FFF;' . "font-family: 'Open Sans', 'sans-serif';". '"'  . " href=\""._URL."/tags/".$row['safe_tag']."/\">". "&nbsp;" . $row['tag'] . "&nbsp;" ."</a>" ;
			}
			else
			{
				$tags[$id]['href'] ="<a " . 'style="background-color:#3881FF;color:#FFF;' . "font-family: 'Open Sans', 'sans-serif';" . '"'  . " href=\""._URL."/tag.php?t=".$row['safe_tag']."\">". "&nbsp;" .$row['tag'] . "&nbsp;" ."</a>";
			}
		}
	}
	return $tags;
