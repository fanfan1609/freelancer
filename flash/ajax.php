			...
			case 'downvote':
				...
				if (preg_match('/([0-9a-zA-Z-]{5,20})/', $video_id) != 0)
				{
					...
					if (is_array($video))
					{
						...
						if ( $userdata['id'] == $video['author_user_id'])
						{
							echo json_encode(array('success' => false,
												   'msg' => 'Sorry, you cannot rate your own videos.'
												  ));
							exit();
						}
						...
						switch ($action)
						{
							...
							case 'upvote':
								$vote_value = 1;
								$reputation_value = 10;
								$sql = "UPDATE users SET reputation = reputation + ".$reputation_value." 
										WHERE id = '". $video['author_user_id'] ."'";
								mysql_query($sql);
							break;
	...
	if(!$captchaResult->is_valid) {
		$errors['recaptcha'] = 'Enter the correct captcha code.';
				...
				if (count($errors) == 0)
				{
					...
					$category = ($category == '' || $category =='0') ? 37 : $category;
