...
function top_videos($sortby = "views", $limit = 7)
{
	...
	switch ($sortby)
	{
		...
		case 'views':
			...
			$sql = "SELECT uniq_id  
					FROM pm_videos 
					WHERE category = '37' AND added <= '". time() ."' 
...
function request_video($unique_id = '', $page = "detail", $autoplay = false)
{
	...
	$sql = "SELECT pm_videos.*, pm_videos_urls.mp4, pm_videos_urls.direct 
			...
			WHERE pm_videos.uniq_id = '". $unique_id ."' AND pm_videos.category = '37'";
    ...
    $video['author_entireweb_refer_code'] = $author_data['entireweb_refer_code'];
    $video['author_reputation'] = $author_data['reputation'];
    $video['author_public_key'] = $author_data['public_key'];
    $video['author_image'] = $author_data['image'];
...
function get_video_list($orderby = '', $sort = '', $start = 0, $limit = 5, $category_id = 0, $video_ids = array(), $uniq_ids = array()) 
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
        ...
        $list[$i]['author_entireweb_refer_code'] = $author_data['entireweb_refer_code'];
        $list[$i]['author_reputation'] = $author_data['reputation'];
        $list[$i]['author_public_key'] = $author_data['public_key'];
        $list[$i]['author_image'] = $author_data['image'];
...
function get_related_video_list($category_id = 0, $video_title = '', $limit = 5, $video_id = 0)
{
	...
	$sql = "SELECT id
			...
			WHERE MATCH (video_title) AGAINST ('". addslashes($video_title) ."')
			  AND category = '37' AND added <= '". time() ."' 
			  AND id != $video_id
			LIMIT 0, $limit";
	...
	if (mysql_num_rows($result) == 0) // backup method; just serve something
	{
		$sql = "SELECT id 
				FROM pm_videos 
				WHERE category = '37' AND (category LIKE '%,$category_id,%' 
...
function get_top_rated_video_list($category_id = 0,  $limit = 5, $video_id = 0)
{
	...
	$sql = "SELECT pm_videos.id 
			...
			WHERE (pm_videos.category LIKE '%,$category_id,%' 
					...
					OR pm_videos.category LIKE '$category_id,%' 
					OR pm_videos.category LIKE '$category_id') AND pm_videos.category = '37'
	...
	if ( ! $result || ! $total_items)
	{
		$sql = "SELECT id  
				FROM pm_videos 
				WHERE category = '37' AND category = '".$category_id."'
				  AND added <= '". time() ."' 
				  AND id != $video_id 
				ORDER BY site_views DESC  
				LIMIT ".$limit;
...
function make_voth() {
	...
	$sql = "SELECT COUNT(*) as total 
			FROM pm_videos 
			WHERE category = '37' AND featured = '1'
			AND added <= '". time() ."'";
	...
	while ( ! $found )
	{
		if ($total_featured)
		{
			...
			$sql = "SELECT uniq_id 
					FROM pm_videos 
					WHERE featured = '1' AND category = '37'
					AND added <= '". time() ."' 
					LIMIT $rand_from, 1";
		}
		else
...
function validate_email($email) {
	...
	$sql = "SELECT email FROM users WHERE email = '" . str_replace("\'", "''", $email) . "'";
...
function get_comment_list($vid, $page =  1, $get_latest_single = false, $order_by = '', $limit = 10)
{
	...
	$sql = "   SELECT pm_comments.*, users.name, users.gender, users.avatar, users.power, users.public_key
			     FROM pm_comments 
			LEFT JOIN users ON (pm_comments.user_id = users.id)
	...
	if (count($comment_list) > 0)
	{
		foreach ($comment_list as $k => $c)
		{
			...
			if ($c['user_id'] == 0)
			{
                ...
                $comment_list[$k]['public_key'] = $c['public_key'];
			}
			elseif ($c['name'] == '')
			{
				$comment_list[$k]['name'] = $c['username'];
...
function show_thumb($uniq_id, $t_id = 1, $video_data = false)
{
	if(_THUMB_FROM == 1) 	//	Outsource
	{
		...
		{
			$q = @mysql_query("SELECT yt_id, yt_thumb, source_id FROM pm_videos WHERE category = '37' AND uniq_id = '".$uniq_id."'");
...
function makevideolink($uniq_id, $video_title = '', $video_slug = '')
{
	...
	if (_SEOMOD == 1) 
	{
		...
		{
			$query 	= @mysql_query("SELECT video_title, video_slug FROM pm_videos WHERE uniq_id = '".$uniq_id."' AND category = '37'");
...
function videosplaying($limit = 4) 
{
	...
	$sql = "  SELECT id FROM pm_videos 
			   WHERE category = '37' AND lastwatched > '".$time_diff."' 
...
function generate_smart_pagination($page = 1, $totalitems, $limit = 15, $adjacents = 1, $targetpage = "/", $pagestring = "&page=", $seomod = 0)
{
	...
	$seo_url_regex = '/(index|browse-|most-viewed|recent|memberlist|search|tag|tags\/)(.*?)([^(\.\/\?|\&|$)]*)/';
...
function get_video_tags($uniq_id = '', $make_links = 1)
{
	...
	while($row = mysql_fetch_assoc($result))
	{
		...
		if($make_links != 0)
		{
			if(_SEOMOD == 1)
			{
				$tags[$id]['href'] ="<a " . 'style="background-color:#3881FF;border-bottom: 1px solid #3881FF;border-right: 1px solid #3881FF;color:#FFF;' . "font-family: 'Open Sans', 'sans-serif';". '"'  . " href=\""._URL."/tags/".$row['safe_tag']."/\">". "&nbsp;" . $row['tag'] . "&nbsp;" ."</a>" ;
			}
			else
			{
				$tags[$id]['href'] ="<a " . 'style="background-color:#3881FF;border-bottom: 1px solid #3881FF;border-right: 1px solid #3881FF;color:#FFF;' . "font-family: 'Open Sans', 'sans-serif';" . '"'  . " href=\""._URL."/tag.php?t=".$row['safe_tag']."\">". "&nbsp;" .$row['tag'] . "&nbsp;" ."</a>";
			}
		}
	}
	return $tags;
...
function stats()
{
	...
	$sql = "SELECT COUNT(*) as total FROM pm_videos WHERE category = '37' AND added >= '". ($now - $sevend) . "' AND added <= '". time() ."'";
	$result = @mysql_query($sql);
	...
	//	online users
	$sql = "SELECT COUNT(*) as total FROM users WHERE last_signin > '". ($now - $fivem) ."'";
	$result = @mysql_query($sql);
	$r 	= @mysql_fetch_assoc($result);
		...
		if ($category_id != '')
		{
			if(strpos($category_id,',') !== false){
				...
				$sql_where .= "(category LIKE '$category_id'
							 	  OR category LIKE '$category_id,%'
							 	  OR category LIKE '%,$category_id'
							 	  OR category LIKE '%,$category_id,%') AND category = '37' ";
			}
		}
	}
...
function smarty_get_video_meta_list($params, &$smarty)
{
	...
	if ($params['uniq_id'] != '')
	{
		...
		else
		{
			$sql = "SELECT id  
					FROM pm_videos 
					WHERE category = '37' AND uniq_id = '". $params['uniq_id'] ."'";
			if ($result = mysql_query($sql))
			{
				$row = mysql_fetch_assoc($result);
				mysql_free_result($result);
