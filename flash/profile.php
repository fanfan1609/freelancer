...
$query_total = @mysql_query("SELECT COUNT(*) as total FROM pm_videos WHERE submitted = '".$username."' AND added <= '". time() ."' AND category = '37'");
...
$sql = "SELECT uniq_id 
		FROM pm_videos 
		WHERE submitted = '". secure_sql($username) ."' AND category = '37'
		  AND added <= '". time() ."' 
		ORDER BY id DESC
		LIMIT 16";
...
$meta_title = sprintf($lang['profile_title'], $full_name, $username, 'Travel Videos | FlockShare');
$meta_description = sprintf($lang['profile_description'], $full_name, $username).' on '.'Travel Videos | FlockShare'.'. '.fewchars($about, 40);
// end
