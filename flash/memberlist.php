...
$meta_title .= ' — Travel Videos | FlockShare';
$meta_title = sprintf($meta_title, 'Travel Videos | FlockShare');
...
$x = 2;
$sql = "SELECT COUNT(*) as total_users FROM users WHERE power IN ('".U_ACTIVE."', '".U_ADMIN."', '".U_MODERATOR."', '".U_EDITOR."')"; 
...
else
{
	...
	$sql = str_replace('COUNT(*) as total_users', '*', $sql);
	$result = mysql_query($sql); 
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$user_list[$i] = $row;
		$user_list[$i]['avatar_url'] = get_avatar_url($row['avatar'], $row['id']);
		$user_list[$i]['profile_url'] = _URL."/profile."._FEXT."?u=".$row['username'];
		$user_list[$i]['last_seen'] = time_since($row['last_signin'], true).' '.$lang['ago'];
		$user_list[$i]['is_online'] = $is_online;
		$user_list[$i]['country_label'] = countryid2name($row['country']);
		$user_list[$i]['status_label'] = ($is_online) ? $lang['memberlist_on'] : $lang['memberlist_off'];
		$user_list[$i]['user_is_banned'] = ($banned['user_id'] == $row['id']) ? true : false;
	}
	mysql_free_result($result);
