...
$modframework->trigger_hook('recent_top');
$page = (int) $_GET['page'];
if ( ! $page)
{
	$page = 1;
...
switch ($date)
{
	...
	case 'month':
		...
		$sql = "SELECT COUNT(*) as total_found 
				FROM pm_videos 
				WHERE added >= '". $time_start ."' AND category = '37'
				  AND added <= '". $time_end ."'";
...
if ($total_videos > 0)
{
	switch ($date)
	{
		...
		case 'month':
			...
			$sql = "SELECT id  
					FROM pm_videos 
					WHERE category = '37' AND added >= '". $time_start ."' 
				  	  AND added <= '". $time_end ."' 
					ORDER BY added DESC 
					LIMIT ". $from .",". $limit;	
		break;
...
{
	$filename = (_SEOMOD) ? 'recent.html' : 'recent.php';
...
$meta_title = 'Recent Zoology Videos | FlockShare';
if(!empty($date)) {
...
$modframework->trigger_hook('recent_bottom');
