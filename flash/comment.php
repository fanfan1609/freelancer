...
if ( ($logged_in == 1) || ($logged_in == 0 && $config['guests_can_comment'] == 1))
{
	if (isset ($_POST['vid']))
	{
		...
		if (strpos($vid, 'article-') !== false)
		{
			...
			if ( ! $vid)
			{
				exit(json_encode(array("cond" => false, "msg" => $lang['comment_msg4'])));
			}
		...
		else
		{
			...
			if ($user == '')
			{
				exit (json_encode(array("cond" => false, "msg" => $lang['comment_msg1'])));
			}
			else
			{
				$sql = "SELECT username FROM users WHERE power = '".U_ADMIN."'";
		...
		if ($comment != '')
		{
			...
			else
			{
				...
				else
				{
					...
					$message = json_encode(array("cond" => true, "msg" => $lang['comment_msg6'], "preview" => true, "html" => $html));
					if (_MOD_SOCIAL)
					{
						$object_data = array();
