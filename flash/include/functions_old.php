...
function top_videos($sortby = "views", $limit = 7)
{
	...
	switch ($sortby)
	{
		...
		case 'views':
			...
			$sql = "SELECT uniq_id  
					FROM pm_videos 
					WHERE category = '27' AND added <= '". time() ."' 
...
function request_video($unique_id = '', $page = "detail", $autoplay = false)
{
	...
	$sql = "SELECT pm_videos.*, pm_videos_urls.mp4, pm_videos_urls.direct 
			...
			WHERE pm_videos.uniq_id = '". $unique_id ."' AND pm_videos.category = '27'";
...
function get_related_video_list($category_id = 0, $video_title = '', $limit = 5, $video_id = 0)
{
	...
	$sql = "SELECT id
			...
			WHERE MATCH (video_title) AGAINST ('". addslashes($video_title) ."')
			  AND category = '27' AND added <= '". time() ."' 
			  AND id != $video_id
			LIMIT 0, $limit";
	...
	if (mysql_num_rows($result) == 0) // backup method; just serve something
	{
		$sql = "SELECT id 
				FROM pm_videos 
				WHERE category = '27' AND (category LIKE '%,$category_id,%' 
...
function get_top_rated_video_list($category_id = 0,  $limit = 5, $video_id = 0)
{
	...
	$sql = "SELECT pm_videos.id 
			...
			WHERE (pm_videos.category LIKE '%,$category_id,%' 
					...
					OR pm_videos.category LIKE '$category_id,%' 
					OR pm_videos.category LIKE '$category_id') AND pm_videos.category = '27'
	...
	if ( ! $result || ! $total_items)
	{
		$sql = "SELECT id  
				FROM pm_videos 
				WHERE category = '27' AND category = '".$category_id."'
				  AND added <= '". time() ."' 
				  AND id != $video_id 
				ORDER BY site_views DESC  
				LIMIT ".$limit;
...
function make_voth() {
	...
	$sql = "SELECT COUNT(*) as total 
			FROM pm_videos 
			WHERE category = '27' AND featured = '1'
			AND added <= '". time() ."'";
	...
	while ( ! $found )
	{
		if ($total_featured)
		{
			...
			$sql = "SELECT uniq_id 
					FROM pm_videos 
					WHERE featured = '1' AND category = '27'
					AND added <= '". time() ."' 
					LIMIT $rand_from, 1";
		}
		else
...
function show_thumb($uniq_id, $t_id = 1, $video_data = false)
{
	if(_THUMB_FROM == 1) 	//	Outsource
	{
		...
		{
			$q = @mysql_query("SELECT yt_id, yt_thumb, source_id FROM pm_videos WHERE category = '27' AND uniq_id = '".$uniq_id."'");
...
function makevideolink($uniq_id, $video_title = '', $video_slug = '')
{
	...
	if (_SEOMOD == 1) 
	{
		...
		{
			$query 	= @mysql_query("SELECT video_title, video_slug FROM pm_videos WHERE uniq_id = '".$uniq_id."' AND category = '27'");
...
function videosplaying($limit = 4) 
{
	...
	$sql = "  SELECT id FROM pm_videos 
			   WHERE category = '27' AND lastwatched > '".$time_diff."' 
...
function stats()
{
	...
	$sql = "SELECT COUNT(*) as total FROM pm_videos WHERE category = '27' AND added >= '". ($now - $sevend) . "' AND added <= '". time() ."'";
	$result = @mysql_query($sql);
		...
		if ($category_id != '')
		{
			if(strpos($category_id,',') !== false){
				...
				$sql_where .= "(category LIKE '$category_id'
							 	  OR category LIKE '$category_id,%'
							 	  OR category LIKE '%,$category_id'
							 	  OR category LIKE '%,$category_id,%') AND category = '27' ";
			}
		}
	}
...
function smarty_get_video_meta_list($params, &$smarty)
{
	...
	if ($params['uniq_id'] != '')
	{
		...
		else
		{
			$sql = "SELECT id  
					FROM pm_videos 
					WHERE category = '27' AND uniq_id = '". $params['uniq_id'] ."'";
			if ($result = mysql_query($sql))
			{
				$row = mysql_fetch_assoc($result);
				mysql_free_result($result);
