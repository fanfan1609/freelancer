...
function get_followers_list($leader_id, $start = 0, $limit = 30)
{
	...
	$sql = "SELECT f.follower_id, u.username, u.name, u.gender, u.country, u.reg_date, u.last_signin, u.power, u.about, u.avatar, u.followers_count, u.following_count 
			FROM users_follow f 
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$followers[$row['follower_id']]['profile_url'] = _URL."/profile."._FEXT."?u=".$row['username'];
...
function get_following_list($follower_id = -1, $start = 0, $limit = 30)
{
	...
	$sql = "SELECT f.user_id, u.username, u.name, u.gender, u.country, u.reg_date, u.last_signin, u.power, u.about, u.avatar, u.followers_count, u.following_count 
			FROM users_follow f 
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$leaders[$row['user_id']]['profile_url'] = _URL."/profile."._FEXT."?u=".$row['username'];
...
function get_following_activity_stream($start = 0, $limit = 30)
{
	...
	$sql = "SELECT pm_activity.*, users.username, users.name, users.avatar, users.power  
			FROM pm_activity 
			JOIN users ON (pm_activity.user_id = users.id)  
			WHERE (
...
function get_user_activity($user_id, $start = 0, $limit = 30)
{
	...
	{
		$sql = "SELECT id, username, name, avatar, power 
				FROM users 
				WHERE id = '". $user_id ."'";
		$result = mysql_query($sql);
...
function parse_activity_list(&$the_list)
{
	...
	foreach ($the_list as $k => $row)
	{
		...
		$the_list[$k]['profile_url'] = _URL.'/profile.'. _FEXT .'?u='. $row['username'];
		...
		if ($row['object_id'] != 0)
		{
			...
			switch ($row['object_type'])
			{
				case ACT_OBJ_USER:
					...
					$meta['object']['profile_url'] = _URL.'/profile.'. _FEXT .'?u='. $meta['object']['username'];
		...
		if ($row['target_id'] != 0)
		{
			switch ($row['target_type'])
			{
				case ACT_OBJ_USER:
					...
					$meta['target']['profile_url'] = _URL.'/profile.'. _FEXT .'?u='. $meta['target']['username'];
...
function admin_get_activities($start = 0, $limit = 20, $page = 1, $filter = '', $filter_value = '')
{
	...
	$sql = "SELECT pm_activity.*, users.username, users.name, users.avatar, users.power  
			FROM pm_activity 
			LEFT JOIN users ON (pm_activity.user_id = users.id)
			$sql_where 
			ORDER BY time DESC 
			LIMIT $start, $limit";
...
function get_latest_notifications($start = 0, $limit = 7) 
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		{
			$meta['from_userdata']['profile_url'] = _URL .'/profile.'. _FEXT .'?u='. $meta['from_userdata']['username'];
		}
		else
		{
...
function suggest_who_to_follow($start_from = 0, $limit = 10)
{
	...
	$sql = "SELECT id, username, name, gender, country, reg_date, last_signin, power, avatar, followers_count, following_count 
			FROM users 
			WHERE power IN ('".U_ACTIVE."', '".U_ADMIN."', '".U_MODERATOR."', '".U_EDITOR."') ";
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$row['reg_date'] = (int) $row['reg_date'];
		$row['last_signin'] = (int) $row['last_signin'];
		$row['followers_count'] = (int) $row['followers_count'];
		$row['following_count'] = (int) $row['following_count'];
		
		$is_online = islive($row['last_signin']);
		
		$user_list[$id]['user_id'] = $id;
		$user_list[$id] = $row;
		$user_list[$id]['avatar_url'] = get_avatar_url($row['avatar'], $row['id']);
		$user_list[$id]['profile_url'] = _URL."/profile."._FEXT."?u=".$row['username'];
		$user_list[$id]['last_seen'] = time_since($row['last_signin'], true).' '.$lang['ago'];
		$user_list[$id]['is_online'] = $is_online;
		$user_list[$id]['country'] = $row['country'];
		$user_list[$id]['country_label'] = countryid2name($row['country']);
		$user_list[$id]['status_label'] = ($is_online) ? $lang['memberlist_on'] : $lang['memberlist_off'];
