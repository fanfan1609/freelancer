...
$meta_title = $lang['create_account'].' — Travel Videos | FlockShare' ;
...
if( isset($_POST['Register']) && $config['allow_registration'] == '1')
{
	...
	if( $nr_errors )
	{
		...
		$sql = "INSERT INTO users (username, password, email, name, gender, country, reg_date, last_signin, reg_ip, about, power, activation_key) VALUES ";
		...
		if ($config['account_activation'] == AA_ADMIN)
		{
			...
			require_once("include/class.phpmailer.php");
				...
				$mailsubject = sprintf($lang['mailer_subj6'], 'Travel Videos | FlockShare');
				
				$array_content[]=array("mail_username", $username);  
				$array_content[]=array("mail_password", $pass);
				$array_content[]=array("mail_ip", $ip);
				$array_content[]=array("mail_sitename", 'Travel Videos | FlockShare');
		...
		{	
			...
			require_once("include/class.phpmailer.php");
				...
				//*** DEFINING E-MAIL VARS
				$mailsubject = sprintf($lang['mailer_subj4'], 'Travel Videos | FlockShare');
		...
		{	
			...
			require_once("include/class.phpmailer.php");
				...
				//*** DEFINING E-MAIL VARS
				$mailsubject = sprintf($lang['mailer_subj1'], 'Travel Videos | FlockShare');
