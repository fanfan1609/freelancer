	...
	switch ($search_type)
	{
		...
		case 'video':
			...
			/*$sql = "SELECT SQL_CALC_FOUND_ROWS uniq_id, category, video_title, site_views, featured, MATCH(video_title) AGAINST ('$searchstring') AS score 
					FROM pm_videos 
			        WHERE category = '37' AND  added <= '". time() ."' AND MATCH(video_title) AGAINST ('$searchstring')
					ORDER BY score DESC 
					LIMIT ".$from.", ".$limit;*/
			...
			if ($num_res == 0 && (strlen($searchstring) >= 2))
			{
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS id 
						FROM pm_videos
						WHERE category = '37' AND added <= '". time() ."' AND (". $where .") 
						LIMIT ".$from.", ".$limit;
				$result = @mysql_query($sql);
		...
		case 'user':
			...
			if (username_to_id($searchstring) > 0)
			{
				$sql = "SELECT COUNT(*) as total  
						 FROM pm_videos
						WHERE category = '37' AND submitted = '". $searchstring ."'
						  AND added <= '". time() ."' 
						ORDER BY id DESC";
				$result = mysql_query($sql);
				$row = mysql_fetch_assoc($result);
