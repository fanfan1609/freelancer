...
$meta_title = $lang['login']." | FlockShare";
$meta_description = '';
// end
...
switch($mode){ 
	...
	case 'activate':
		...
		{
			$sql = "SELECT * FROM users WHERE id = '".secure_sql($user_id)."'";
			...
			if($mode == 'activate')
			{
				...
				{
					$sql = "UPDATE users SET power = '".U_ACTIVE."', activation_key = '' WHERE id = '".$user['id']."'";
					$result = @mysql_query($sql);
					$dobreak = false;
			...
			elseif($mode == 'pwdreset')
			{
				...
				{
					$sql = "UPDATE users SET password = '".$user['new_password']."', activation_key = '', new_password = '' WHERE id = '".$user['id']."'";
					$dobreak = false;
