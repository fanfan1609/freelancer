...
function generateFeed($homeURL, $title, $version = '2.0', $items = array(), $category_data = '', $feed = '') 
{
	...
	else
	{
		...
		{
			$rss_title = sprintf($lang['homepage_title'], 'Travel Videos | FlockShare');
		}
		else
		{
	...
	if (count($items) > 0) 
	{
		foreach ($items as $k => $item)
		{
			...
			else
			{
				...
				$media_content_url = '';
				if ((strpos($item['url_flv'], 'http://') !== false && $item['source_id'] != 0) || ($item['source_id'] == 1))
				{
					$tmp_parts = explode('.', $item['url_flv']);
					$ext = array_pop($tmp_parts);
					$ext = strtolower($ext);
			...
			if ($feed != 'articles')
			{
				...
				$output .= ($media_content_url != '') ? ' url="'. $media_content_url .'" ' : '';
...
{ 
	...
	else
	{
		$sql = "SELECT * 
				FROM pm_videos 
				WHERE category = '37' AND (category LIKE '%,$category,%' 
				   OR category LIKE '%,$category' 
				   OR category LIKE '$category,%' 
				   OR category='$category') 
				  AND added <= '". time() ."' 
...
echo generateFeed($url ,'Travel Videos | FlockShare', $rssVersion, $items, $cat_data, $rss_for);
?>